...
export const ToneStudio = () => {
  ...
  useEffect(() => {
    ...
    void supabase.from('tone_settings').select('sample_emails, tone_level').eq('workspace_id', activeWorkspace.id).maybeSingle().then(({ data }) => {
      if (!data) return;
      setToneSample(data.sample_emails ?? '');
      setToneLevel(data.tone_level ?? 2);
    });
  }, [activeWorkspace?.id]);

  const handleSave = async () => {
    if (!activeWorkspace || !toneSample.trim()) { addToast('Add a sample email before saving.', 'warning'); return; }
    const { error } = await supabase.from('tone_settings').upsert({ workspace_id: activeWorkspace.id, sample_emails: toneSample.trim(), tone_level: toneLevel, ai_prompt: `Tone level ${toneLevel}`, updated_at: new Date().toISOString() });
    addToast(error ? error.message : 'Tone settings saved.', error ? 'warning' : 'success');
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
            ...
            <div className="space-y-6">
              ...
              <button onClick={() => void handleSave()} className="w-full rounded-xl border border-gray-200 bg-white py-3 text-sm font-bold text-gray-700 transition hover:border-brand-blue hover:text-brand-blue">Save tone settings</button>
            </div>
          </div>
