...
async function listWorkspaceRows(table: 'opportunities' | 'accounts' | 'decisions' | 'invoices', workspaceId: string) {
  const query = supabase.from(table).select('*').eq('workspace_id', workspaceId).limit(10);
  ...
  const handleSend = async (text: string) => {
    ...
    if (!activeWorkspace?.id) {
      ...
      if (lower.includes('opportunit')) {
        const opps = (await listWorkspaceRows('opportunities', activeWorkspace.id)) as Opportunity[];
        if (opps.length > 0) {
          innerContent = (
            <div className="space-y-2">
        ...
        const accounts = (await listWorkspaceRows('accounts', activeWorkspace.id)) as Account[];
        ...
        const decisions = (await listWorkspaceRows('decisions', activeWorkspace.id)) as Decision[];
        ...
        const invoices = (await listWorkspaceRows('invoices', activeWorkspace.id)) as Invoice[];
        const pending = invoices.filter((i: Invoice) => i.status === 'pending');
        innerContent = `You have ${pending.length} pending invoices totaling $${pending.reduce((s: number, i: Invoice) => s + Number(i.amount), 0).toLocaleString()}.`;
      }
