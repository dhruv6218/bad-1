...
export const Settings = () => {
  ...
  const handleDeleteAccount = () => {
    addToast('Account deletion is handled through a verified support request so data is not removed accidentally.', 'warning');
  };
  ...
  const handleSaveProfile = async () => {
    ...
    const { error } = await supabase.from('profiles').upsert({ id: user.id, full_name: profileName.trim(), updated_at: new Date().toISOString() });
    const workspaceResult = workspaceName.trim() && activeWorkspace ? await updateWorkspaceName(workspaceName) : { error: null };
    if (error || workspaceResult.error) addToast(error?.message || workspaceResult.error || 'Could not save profile.', 'warning');
              ...
              <div className="p-6 space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input type="text" value={profileName} onChange={(event) => setProfileName(event.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-brand-blue transition-all" />
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <input type="text" value={workspaceName} onChange={(event) => setWorkspaceName(event.target.value)} placeholder="e.g. Acme Design Studio" className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-brand-blue transition-all" />
                </div>
                <div className="flex justify-end pt-4">
                  <button onClick={() => void handleSaveProfile()} disabled={isSavingProfile} className="bg-gray-900 text-white px-6 py-2.5 rounded-xl font-bold text-sm hover:bg-black transition-colors shadow-sm disabled:opacity-60">
