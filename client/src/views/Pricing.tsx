...
export const Pricing = () => {
  ...
  const handleCheckout = async (tier: any) => {
    ...
    try {
      ...
      if (!session) throw new Error('Please sign in again before starting checkout.');
      const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/create-checkout-v2`, {
        method: 'POST',
        headers: {
      ...
      addToast(error instanceof Error ? error.message : 'Could not start checkout.', 'warning');
    } finally {
      setLoadingTier(null);
    }
  };
